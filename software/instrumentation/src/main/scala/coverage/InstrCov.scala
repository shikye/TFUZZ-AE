...
class InstrCov(mod: DefModule, mInfo: moduleInfo, extModules: Seq[String], val maxStateSize: Int = 15, val covSumSize: Int=16) {
  ...
  def instrument(): DefModule = {
    ...
    val defRegister = (name: String, info: String, clock_name: String, width: Int) =>
      DefRegister(FileInfo(StringLit(info)), //注释
        name, UIntType(IntWidth(width)), //名称+类型
        clockRef(clock_name), UIntLiteral(0, IntWidth(1)), initReg(name, width)) //时钟+复位+初始化
    ...
    mod match {
      case mod: Module => {
        ...
        if (regStateSize != 0 && hasCNR) {
          ...
          val covMap = defRegister(covMapName, s"Coverage map for ${mName}",
            clockName, covMapSize)
          ...
          // val covMaprdshr = DoPrim(Dshr, 
          //   Seq(WRef(covMap), WRef(regState)), 
          //   Seq(), 
          //   UIntType(IntWidth(covMapSize)))


          // // val constone = DefWire(NoInfo, "constone", UIntType(IntWidth(1)))
          // // val constoneCon = Connect(NoInfo, WRef(constone), UIntLiteral(1))

          // val covMapIndexregState = DoPrim(And,
          //   Seq(covMaprdshr, (UIntLiteral(1))),
          //   Seq(),
          //   UIntType(IntWidth(1)))

          
          
          // val covSumAdd = DoPrim(Not,
          //   Seq(covMapIndexregState),
          //   Seq(),
          //   UIntType(IntWidth(1))
          // )

          val covMapshl = DoPrim(Dshl, 
            Seq((UIntLiteral(1)), WRef(regState)), 
            Seq(), 
            UIntType(IntWidth(covMapSize)))

          val covMapHas = DoPrim(And, 
            Seq(covMapshl, WRef(covMap)),
            Seq(),
            UIntType(IntWidth(1))
          )

          val MidOr = DoPrim(Orr, 
            Seq(covMapHas),
            Seq(),
            UIntType(IntWidth(1))
          )
          ...
          val covMapwr = DoPrim(Or, 
            Seq(covMapshl, WRef(covMap)), 
            Seq(), 
            UIntType(IntWidth(covMapSize)))

          val covMapIndexPluscovSum = DoPrim(Add, 
            Seq(covSumAdd, WRef(covSum)), 
            Seq(), 
          ...
          val covMapwrCon = Connect(NoInfo, WRef(covMap), covMapwr)
          val covSumwrCon = Connect(NoInfo, WRef(covSum), covMapIndexPluscovSum)

          val wrCons = ListBuffer[Statement]()
          wrCons.append(covMapwrCon)
          wrCons.append(covSumwrCon)
          // wrCons.append(constoneCon)
          ...
          val newBlock = Block((stmts :+ regState :+ covMap :+ covSum) 
            ++ stConnections ++ wrCons ++ptConnections)

          mInfo.regState = Some(regState)
          mInfo.covSum = Some(covSum)
          mInfo.covMap = Some(covMap)
